...
model = TR4KW.load_model()
segmentor,postagger = filter_all.load_model()

filter_all_list = filter_all.words_filter_all(raw_text,segmentor,postagger)
sorted_words = TR4KW.filter_keywords(TR4KW.sort_words(filter_all_list,filter_all_list,model,window=2))
#输出结果
with open("wor2vecKW.csv","wb") as fh:
...
sorted_words_tr = TR4KW.filter_keywords(sort_words(filter_all_list,filter_all_list))
#out to file
with open("keywords_out.csv","w") as fh:
    csvwritter = csv.writer(fh)
    for ele in sorted_words_tr:
        csvwritter.writerow((str(ele.weight),ele.word))

#release
TR4KW.release_model(model)
segmentor.release()
postagger.release()
print "finish"
